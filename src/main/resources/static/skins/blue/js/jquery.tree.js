	...
	return $(this).each(function(){
		...
		tree
			...
			.bind('collapse',function(event){
				...
				target.next().slideUp(150, function(){
					...
					if($(this).prev().attr('name')=="tree-1"){
						$(this).prev().removeClass('tree-one-active');
					}
					if($(this).prev().attr('name')=="tree-2"){
							$(this).prev().removeClass('tree-two-active');
		...
		tree	
			.click(function(event){
				...
				if($(event.target).attr("name")=="tree-1"){
					$(event.target).attr('tabindex','0').addClass('tree-one-active');
					$(event.target).each(function(){
					var aa=$(this).siblings().children().children().children().children().hasClass("tree-three-active");//此三级下是否有tree-three-active
					var bb=$(this).siblings().children().children().hasClass("tree-two-active-2");//此二级下是否有tree-two-active-2
						if((aa==true || bb==true)){ //两种情况有一个满足 此加背景并且改变文字颜色
							$(this).attr('tabindex','0').addClass('tree-one-hide-bg');
						}
					})
					
				}
				if($(event.target).attr("name")=="tree-2"){
					$(event.target).each(function(){
						var aa=$(this).siblings().children().children().hasClass("tree-three-active");//此三级下是否有tree-three-active
						if(aa==true){
							$(this).attr('tabindex','0').addClass('tree-two-hide-bg');
						}
						if($(this).siblings().length==0){//如果二级菜单无子级
							tree.find('[name=tree-1]').removeAttr('style');//一级去掉style样式
							tree.find('[name=tree-1]').removeClass('tree-one-hide-bg');//二级去掉样式tree-one-hide-bg
							tree.find('[name=tree-2]').removeClass('tree-two-active-2');//二级去掉样式tree-two-active-2
							tree.find('[name=tree-2]').removeAttr('style');//二级去掉style样式
							tree.find('[name=tree-2]').removeClass('tree-two-hide-bg');//二级去掉样式tree-two-hide-bg
							tree.find('[name=tree-3]').attr('tabindex','-1').removeClass('tree-three-active');//三级去掉样式tree-three-active
							$(this).attr('tabindex','0').addClass('tree-two-active-2');
						}
						else{
							$(this).attr('tabindex','0').addClass('tree-two-active');
						}
					
				});
				}
				if($(event.target).attr("name")=="tree-3"){
				$(event.target).each(function() {
					tree.find('[name=tree-1]').removeAttr('style');
					tree.find('[name=tree-1]').removeClass('tree-one-hide-bg');//二级去掉样式tree-one-hide-bg
					tree.find('[name=tree-2]').attr('tabindex','-1').removeClass('tree-two-active-2'); 
					tree.find('[name=tree-2]').removeAttr('style'); 	
					tree.find('[name=tree-2]').removeClass('tree-two-hide-bg');//二级去掉样式tree-two-hide-bg
					tree.find('[name=tree-3]').attr('tabindex','-1').removeClass('tree-three-active');	
					$(this).attr('tabindex','0').addClass('tree-three-active');
					$(this).addClass("leftmenucurrent");
			...
			.click(function(event){
				...
				if( target.is('a.tree-parent') || target.is('a.tree-parent-one')){
